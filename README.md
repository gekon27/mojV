...
**HACS 0.15.12 — jeden adaptacyjny panel Szkoła, własne zajęcia, A4, frekwencja z legendą, modułowy pulpit i potwierdzane odpowiedzi przez mojV Auth Helper 0.2.0.**
...
W HACS 0.15.12 obsługiwane są rzeczywiste dane:
...
2. Wybierz `mojV` i zainstaluj wersję **0.15.7** lub nowszą.
...
Panel korzysta wyłącznie z publicznego snapshotu zapisanego w Home Assistant. Zmiana dziecka, widoku lub tygodnia nie powoduje dodatkowego logowania do portalu. HACS 0.15.7 pozwala lokalnie przeglądać dokładnie cztery tygodnie: poprzedni, bieżący oraz dwa kolejne, bez dodatkowego requestu przy zmianie tygodnia.
...
Widoki **Plan**, **Frekwencja** i **Statystyki** udostępniają akcje drukowania. Każda akcja otwiera odizolowany dokument A4 z treścią wybranego widoku, bez paska bocznego i elementów Home Assistanta. Do planu można lokalnie dodać cykliczne zajęcia dodatkowe (np. szachy); nie są one wysyłane do e-dziennika.
...
Przy starcie integracji HACS 0.15.12 w logu powinien pojawić się wpis:

`mojV integration version=0.15.12`
