import "./school-panel.js?v=0.15.14";
...
if (proto && !proto.__mojvLiveModulesPatched) {
  ...
  proto._renderMessages = function (student) {
    ...
    return `<section class="card list-view-card live-module-card" data-view="messages">
      ...
      ${rows.length ? `<div class="data-list">${rows.map((message) => `<article class="data-row message-row ${message.unread ? "unread" : ""}">
      ...
      </article>`).join("")}</div>` : `<div class="mini-empty roomy">Brak wiadomości.</div>`}
    </section>`;
  };
  ...
  proto._renderMeetings = function (student) {
    ...
    return `<section class="card list-view-card live-module-card" data-view="meetings">
      ...
      ${rows.length ? `<div class="data-list">${rows.map((item) => {
      ...
      }).join("")}</div>` : `<div class="mini-empty roomy">Brak zebrań w pobranym zakresie.</div>`}
    </section>`;
  };
  ...
  proto._styles = function () {
    return `${baseStyles.call(this)}
      ...
      .live-module-card .data-row p{margin:7px 0 0;color:var(--primary-text-color);font-size:14px;line-height:1.55;white-space:pre-wrap}.live-module-card .data-row strong{font-size:15px}.live-module-card .data-row span{font-size:13px}
