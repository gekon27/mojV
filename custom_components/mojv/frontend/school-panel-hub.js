...
if (proto && !proto.__mojvExpandedSchoolHubPatched) {
  ...
  proto._mojvApplyDisplaySettings = function () {
    ...
    this.style.setProperty("--mojv-font-scale", String(Math.min(1.35, Math.max(1, scale))));
  ...
  proto._mojvOpenModuleSettings = function () {
    ...
    overlay.querySelector("form").addEventListener("submit", (event) => { event.preventDefault(); const next = new Set([...overlay.querySelectorAll("input:checked")].map((input) => input.value)); const form = new FormData(event.currentTarget); const latest = this._mojvDisplaySettings(); latest.fontScale = Number(form.get("fontScale")) || 1.12; latest.themes = latest.themes || {}; latest.themes[this._activeStudentId] = String(form.get("theme") || "system"); localStorage.setItem("mojv.display_settings", JSON.stringify(latest)); this._mojvSaveVisibleModules(next); overlay.remove(); this._renderActiveView(); });
  ...
  proto._styles = function () {
    return `${baseStyles.call(this)}
      :host{font-size:calc(16px * var(--mojv-font-scale,1.12))}:host([data-mojv-theme="blue"]){--mv-accent:#2f8cff}:host([data-mojv-theme="warm"]){--mv-accent:#d97706;--mv-soft:rgba(217,119,6,.10)}:host([data-mojv-theme="dark"]){--mv-card:#111827;--mv-soft:#1f2937;--mv-line:#3b4555}.app-shell{width:min(100%,1560px);max-width:none;padding-inline:clamp(12px,2vw,30px)}.topbar{flex-wrap:wrap}.view-nav,.student-nav{width:100%;max-width:none}.view-content{width:100%;min-width:0}.section-head h2,.data-row strong,.work-copy strong{font-size:calc(1em * var(--mojv-font-scale,1.12))}.data-row span,.data-row small,.work-copy>span,.work-copy p{font-size:calc(.84em * var(--mojv-font-scale,1.12));line-height:1.5}.list-view-card,.final-grades-card,.schoolwork-card{max-width:none}.praise-badge{color:var(--mv-good);background:color-mix(in srgb,var(--mv-good) 18%,transparent)}.workspace-view{display:grid;gap:14px}.workspace-view p{margin:5px 0 0;color:var(--mv-muted)}.workspace-student-card{min-width:0}.workspace-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,220px),1fr));gap:10px;padding:16px}.workspace-grid article{display:grid;gap:5px;padding:13px;border:1px solid var(--mv-line);border-radius:12px;background:var(--mv-soft)}.workspace-grid small{color:var(--mv-muted);font-size:12px}.workspace-grid strong{font-size:16px;line-height:1.35}
