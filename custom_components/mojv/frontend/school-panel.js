class MojVSchoolPanel extends HTMLElement {
        ...
        <header class="topbar">
          <div class="brand-block">
            ...
            <div class="brand-copy">
              <span class="eyebrow">mojV</span>
              <h1>Szkoła</h1>
              <p>Plan, frekwencja, oceny i terminarz</p>
            </div>
          </div>
  ...
  _availableViews(student) {
    ...
    if ((student?.grades || []).length || (student?.final_grades || []).length) views.push(["grades", "Oceny", "5"]);
    if ((student?.schoolwork || []).length) views.push(["schoolwork", "Terminarz", "◆"]);
  ...
  _renderActiveView() {
    ...
    switch (this._activeView) {
      ...
      case "grades":
        ...
        break;
      case "schoolwork":
        content.innerHTML = this._renderSchoolwork(student);
        break;
      case "remarks":
  ...
  _renderGrades(student) {
    ...
    const finals = [...(student.final_grades || [])].sort((a, b) => String(a.subject).localeCompare(String(b.subject), "pl"));
    const finalCards = finals.map((grade) => `<article class="final-grade-card"><div><strong>${this._e(grade.subject)}</strong><small>${grade.period ? `Okres ${this._e(grade.period)}` : "Ocena okresowa"}</small></div><div class="final-values"><span title="Proponowana">${this._e(grade.proposed || "—")}</span><strong title="Końcowa">${this._e(grade.final || "—")}</strong></div></article>`).join("");
    const gradeRows = grades.map((grade) => `<article class="data-row"><div class="grade-badge">${this._e(grade.value)}</div><div><strong>${this._e(grade.subject)}</strong><span>${this._e(grade.description || grade.category || "Bez opisu")}</span>${grade.period ? `<small>Okres ${this._e(grade.period)}</small>` : ""}</div><time>${this._e(this._date(grade.date, true))}</time></article>`).join("");
    return `<div class="grades-layout" data-view="grades">
      ${finals.length ? `<section class="card final-grades-card"><div class="section-head"><div><span class="kicker">Klasyfikacja</span><h2>Proponowane i końcowe</h2></div><span>${finals.length}</span></div><div class="final-grade-grid">${finalCards}</div></section>` : ""}
      <section class="card list-view-card"><div class="section-head"><div><span class="kicker">Oceny</span><h2>Najnowsze wpisy</h2></div><span>${grades.length}</span></div>${grades.length ? `<div class="data-list">${gradeRows}</div>` : `<div class="mini-empty roomy">Brak ocen cząstkowych w pobranym zakresie.</div>`}</section>
    </div>`;
  }

  _renderSchoolwork(student) {
    ...
    const renderRows = (rows) => rows.map((item) => {
      ...
      return `<article class="work-row"><div class="work-date"><strong>${date.toLocaleDateString("pl-PL", { day: "2-digit" })}</strong><span>${date.toLocaleDateString("pl-PL", { month: "short" })}</span></div><div class="work-copy"><div><span class="work-kind">${this._e(this._workKind(item.kind))}</span><strong>${this._e(item.title || this._workKind(item.kind))}</strong></div><span>${this._e(item.subject || "Bez przedmiotu")}</span>${item.description ? `<p>${this._e(item.description)}</p>` : ""}</div><div class="due-pill ${days <= 1 && days >= 0 ? "soon" : days < 0 ? "past" : ""}">${this._e(due)}</div></article>`;
    }).join("");
  ...
  _scheduleSlots(days) {
    ...
    for (const day of days) {
      for (const lesson of day.lessons || []) {
        ...
        if (!slots.has(key)) slots.set(key, { key, startMinute: this._minuteOfDay(lesson.start), endMinute: this._minuteOfDay(lesson.end) });
      }
    }
    return [...slots.values()].sort((a, b) => a.startMinute - b.startMinute || a.endMinute - b.endMinute);
  }

  _slotKey(lesson) { return `${this._minuteOfDay(lesson.start)}|${this._minuteOfDay(lesson.end)}`; }
  ...
  _dayStart(value) { const date = value instanceof Date ? new Date(value) : new Date(value); date.setHours(0, 0, 0, 0); return date; }
  _workKind(value) { return ({ test: "Sprawdzian", quiz: "Kartkówka", class_test: "Klasówka", homework: "Zadanie domowe" })[value] || "Inne"; }
