import "./school-panel-live.js?v=0.15.12";
...
if (proto && !proto.__mojvSchoolHubPatched) {
  ...
  proto._styles = function () {
    return `${baseStyles.call(this)}
      ...
      .hub-detail-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.hub-detail-card{display:grid;gap:6px}.hub-detail-card>:not(.section-head){margin-inline:18px}.hub-detail-card>:last-child{margin-bottom:18px}.hub-detail-card>span,.hub-detail-card>small{color:var(--mv-muted)}
      ...
      .hub-notification-row{position:relative;cursor:help}.hub-notification-row:focus-visible{outline:2px solid var(--mv-accent);outline-offset:2px;border-radius:10px}.hub-notification-row::after{content:attr(data-notification-detail);position:absolute;z-index:8;left:48px;bottom:calc(100% - 6px);max-width:min(540px,calc(100% - 58px));padding:9px 11px;border:1px solid var(--mv-line);border-radius:10px;background:var(--mv-card);box-shadow:0 8px 24px rgba(0,0,0,.2);color:var(--primary-text-color,#fff);font-size:11px;line-height:1.4;white-space:pre-wrap;pointer-events:none;opacity:0;transform:translateY(4px);transition:opacity .14s,transform .14s}.hub-notification-row:hover::after,.hub-notification-row:focus-visible::after{opacity:1;transform:translateY(0)}
