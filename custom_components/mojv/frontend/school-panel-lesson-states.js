...
if (proto && !proto.__mojvLessonStatesPatched) {
  ...
  proto._styles = function () {
    return `${baseStyles.call(this)}
      ...
      .lesson-state-completed{background:color-mix(in srgb,var(--mv-good) 13%,var(--mv-card))!important;border-color:color-mix(in srgb,var(--mv-good) 45%,var(--mv-line))!important;color:var(--primary-text-color,#fff)}.lesson-state-completed .schedule-lesson-meta,.lesson-state-completed .timeline-copy>span{color:color-mix(in srgb,var(--mv-good) 68%,var(--mv-muted))}.lesson-state-completed .lesson-state-label{background:color-mix(in srgb,var(--mv-good) 18%,transparent);color:var(--mv-good)}
